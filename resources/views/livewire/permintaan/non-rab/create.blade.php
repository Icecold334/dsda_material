            ...
            <x-card title="Informasi Permintaan">
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="mt-1 block w-full max-w-[500px]">
                            <livewire:components.select-input wire:model.live="warehouse_id" :options="$warehouses->pluck('name', 'id')"
                                placeholder="-- Pilih Gudang --" :key="'warehouse-select-' . $sudin_id" />
                        </div>
                    </div>
                    ...
                    @else
                        <div class="flex items-center justify-between">
                            ...
                            <div class="mt-1 block w-full max-w-[500px]">
                                <livewire:components.select-input wire:model.live="item_type_id" :options="$itemTypes->pluck('name', 'id')"
                                    placeholder="-- Pilih Tipe Barang --" :disabled="count($items) > 0" :key="'type-select-' . $warehouse_id . '-' . count($items)" />
                                @if (count($items) > 0)
                        ...
                        @if ($item_category_id)
                            <div class="flex items-center justify-between">
                                ...
                                <div class="mt-1 block w-full max-w-[500px]">
                                    <livewire:components.select-input wire:model.live="item_id" :options="$availableItems->mapWithKeys(
                                        fn($item) => [
                                            $item->id =>
                                                $item->spec .
                                                ' (Stok: ' .
                                                ($item->stocks->where('warehouse_id', $warehouse_id)->first()?->qty ??
                                                    0) .
                                                ' ' .
                                ...
                                <div class="mt-1 block w-full max-w-[500px]">
                                    <div class="relative">
                                        @php
                                            ...
                                            $maxStock = $selectedItem
                                                ? $selectedItem->stocks->where('warehouse_id', $warehouse_id)->first()
                                                        ?->qty ?? 0
                                                : 0;
                                            $placeholderText =
                                                $maxStock > 0 ? 'Maks: ' . number_format($maxStock, 2) : '0';
                                        @endphp
                                        <x-text-input id="qty_request" wire:model="qty_request" type="number"
                                            step="0.01" min="0.01" max="{{ $maxStock > 0 ? $maxStock : '' }}"
                                            class="w-full pr-20" placeholder="{{ $placeholderText }}" />
                                        @if ($item_id && $unit)
                                            <span
                                    ...
                                    @if ($item_id && $maxStock > 0)
                                        <p class="mt-1 text-xs text-gray-500">Stok tersedia:
                                            {{ number_format($maxStock, 2) }} {{ $unit }}</p>
                                    @endif
                                </div>
                            </div>
                        @endif
                            ...
                            @foreach ($items as $index => $item)
                                <tr class="even:bg-primary-100 odd:bg-primary-50 border-primary-200">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        {{ number_format($item['qty_request'], 2) }} <span
            ...
            Livewire.on('validation-passed-create', () => {
                showConfirm({
                    ...
                    onConfirm: () => {
                        ...
                        Livewire.dispatch('confirm-save-permintaan');
                    }
                });
            });
        });
