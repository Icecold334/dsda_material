                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="mt-1 block w-full max-w-[500px]">
                                    <livewire:components.select-input wire:model.live="item_id"
                                        :options="$availableItems->mapWithKeys(fn($item) => [$item->id => $item->spec . ' (Stok: ' . ($item->stocks->where('warehouse_id', $warehouse_id)->first()?->qty ?? 0) . ' ' . ($item->category->unit->name ?? '') . ')'])" placeholder="-- Pilih Spesifikasi --"
